...
describe('GainNode', () => {
  ...
  describe('AudioParam', () => {
    ...
    it('sets gain linearly', (done) => {
      ...
      setTimeout(() => {
        expect(
          testComponent.node.gain.value < 6 && testComponent.node.gain.value > 4
        ).toBe(true);
        setTimeout(() => {
    ...
    it('sets gain exponentially', (done) => {
      ...
      setTimeout(() => {
        expect(
          testComponent.node.gain.value < 4 && testComponent.node.gain.value > 2
        ).toBe(true);
        setTimeout(() => {
          expect(Math.round(testComponent.node.gain.value)).toBe(10);
          done();
